 ...
 * Create a dummy.
 *
 * Per Martin Fowler, "Dummy objects are passed around but never actually used.
 ...
 *
 * Per Martin Fowler, "Fake objects actually have working implementations, but
 * usually take some shortcut which makes them not suitable for production (an
 ...
 *
 * Per Martin Fowler, "Mocks are pre-programmed with expectations which form a
 * specification of the calls they are expected to receive. They can throw an
 * exception if they receive a call they don't expect and are checked during
 ...
 * Create a stub.
 *
 * Per Martin Fowler, "Stubs provide canned answers to calls made during the
