...
Deno.test({
  ...
  async fn(): Promise<void> {
    ...
    const firstFailureResult = splitStdout[1].split("at Module.assertEquals")[0] // Output of failing on first test
    const secondFailureResult = splitStdout[1].split("at Object.runTests")[1].split("at Module.assertEquals")[0]
    ...
    asserts.assertEquals(firstFailureResult,
        "\n" +
        "\n" +
        "test_case_1a1\n" +
    ...
    asserts.assertEquals(secondFailureResult,
